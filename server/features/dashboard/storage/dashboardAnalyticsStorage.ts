...
export const dashboardAnalyticsStorage = {
  async getDashboardAnalytics(params: DashboardAnalyticsParams): Promise<DashboardAnalyticsResponse> {
    ...
    const result = await db.execute<{
      ...
      -- Users aggregation (authenticated vs anonymous)
      users_agg AS (
        SELECT 
      ...
      -- Hourly data (uses different time windows)
      tz AS (
      ...
      SELECT 
        ...
        (SELECT json_agg(json_build_object('hour', hour, 'isCurrentHour', is_current_hour, 'isPast', is_past, 'todayCount', today_count, 'lastWeekCount', last_week_count) ORDER BY hour) FROM hourly_agg) as hourly_json
    `);

    const row = result.rows[0];
    
    const parseJsonSafe = <T>(json: string | null | undefined): T[] => {
      if (!json) return [];
      try {
    ...
    const products: ProductItem[] = parseJsonSafe<ProductItem>(row?.products_json);
    const emotions: EmotionItem[] = parseJsonSafe<EmotionItem>(row?.emotions_json);
    const problems: ProblemItem[] = parseJsonSafe<ProblemItem>(row?.problems_json);
    const hourly: HourlyItem[] = parseJsonSafe<HourlyItem>(row?.hourly_json);
    
    const productsTotal = products.reduce((sum, p) => sum + p.count, 0);
    const emotionsTotal = emotions.reduce((sum, e) => sum + e.count, 0);
    const usersTotal = row?.users_total || 0;
    const usersAuthenticated = row?.users_authenticated || 0;

    return {
      products: { items: products, total: productsTotal },
      emotions: { items: emotions, total: emotionsTotal },
      problems: { items: problems, total: row?.problems_total || 0 },
      users: { 
        total: usersTotal, 
