...
function getAuthHeader(): string {
  const email = process.env.ZENDESK_HC_EMAIL;
  const token = process.env.ZENDESK_HC_API_TOKEN;
  
  if (!email || !token) {
    throw new Error("Missing ZENDESK_HC_EMAIL or ZENDESK_HC_API_TOKEN environment variables");
  }
