...
*   **ConversationOrchestrator Pipeline:** Uses a simplified status-based delegation model. The orchestrator only defines which agent is responsible based on status, and each agent controls its own lifecycle. **Status Flow:** `NEW → FINDING_DEMAND → [loop: enrich + search articles + evaluate score] → score OK? → DEMAND_CONFIRMED → PROVIDING_SOLUTION → COMPLETED` or `score not OK? → clarification question → AWAITING_CUSTOMER_REPLY → back to FINDING_DEMAND`. **Key Components:**
