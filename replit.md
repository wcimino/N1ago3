...
*   **Handoff-is-Final Pattern:** When an agent transitions ownership to another agent, it MUST use the `handoffAndReturn()` helper from `shared/helpers.ts`. This ensures no code executes after a handoff, preventing state overwrites. The helper: (1) applies context patches, (2) updates database state, (3) returns the result. Use this for all ownership transitions (e.g., DemandFinder→SolutionProvider, SolutionProvider→Closer, Closer→DemandFinder). State updates within the same owner (e.g., setting `waitingForCustomer=true` while staying in CLOSER) do NOT require this helper.
