...
*   **ConversationOrchestrator Pipeline:** Uses a Coordinator Pattern with 4 phases: **Phase 1: Enrichment** (Summary, Classification, ArticlesAndSolutions) → **Phase 2: Evaluation** (step4_Evaluate determines demandFound/hasCaseSolution) → **Phase 3: Action** (SolutionProvider if hasCaseSolution OR demandFound, else DemandFinder) → **Phase 4: Response** (sends best available response via SendResponse). Key design: SolutionProviderAgent owns case_solution creation via SolutionResolverService when demandFound=true. Status updates use conversationStorage directly to avoid circular dependencies. Each agent has a simple "enabled" toggle as a kill switch.
...
*   **Case Demand Architecture:** The `case_demand` table stores customer demands per conversation, including `articles_and_objective_problems` (matched articles and problems), `interaction_count` (DemandFinder interaction counter), and `status` (demand finder status). Uses unique constraint on `conversation_id`. Storage layer: `caseDemandStorage` in `server/features/ai/storage/`.
