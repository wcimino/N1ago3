...
*   **ConversationOrchestrator Pipeline:** Manages conversation flow using a 2-field state model (`conversation_owner` + `waiting_for_customer`) instead of a complex status enum. The orchestrator dispatches to exactly one agent per customer message and exits immediately after the agent sets `waiting_for_customer=true`. Agents (DemandFinder, SolutionProvider, Closer) control their own lifecycles and ownership transitions. Status values: `NEW`, `FINDING_DEMAND`, `PROVIDING_SOLUTION`, `FINALIZING`, `CLOSED`, `ESCALATED`. Owner values: `demand_finder`, `solution_provider`, `closer`. Supports multiple demands per conversation with interaction counting (max 5 questions before escalation).
