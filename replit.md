...
*   **ConversationOrchestrator Pipeline:** Sequential agent execution pipeline (Step 1: Summary → Step 2: Classification → Step 3: ArticlesAndSolutions → Step 4: StatusController → Step 5: DemandFinder → Step 6: SolutionProvider → Step 7: SendResponse → Step 8: TransferToHuman) that processes customer messages. Each agent has a simple "enabled" toggle as a kill switch.
...
*   **External Sources & Knowledge Base Architecture:** Replicas of external data (e.g., Zendesk articles) are synced manually. Internal Q&A articles stored in `knowledge_base` with `question`, `answer`, `keywords`, `question_variation`, `product_id`, `subject_id`, `intent_id`, and `is_active` (boolean, default false - articles must be activated to be used in chatbot searches). Knowledge base embeddings are stored with content hash. Raw Zendesk data is in `zendesk_articles` with separate `zendesk_article_embeddings`.
