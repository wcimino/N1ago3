...
*   **Authentication System:** Uses Replit Auth (Google Login) with an Access Control List (ACL) based on email domains and an `authorized_users` table. Auth module structure: `server/features/auth/` contains `services/replitAuth.ts` (OIDC setup, session management, login/logout routes), `routes/auth.ts` (user info endpoints), and `storage/authStorage.ts` (authorized users persistence).
