...
*   **Product Catalog Architecture (Dec 2025):** Restructured to use separate `products_catalog` and `subproducts_catalog` tables. Products have `external_id`, `name`, `icon`, `color`. Subproducts reference products via `produto_id` (UUID). This enables external system sync and better UI customization. Note: `conversations_summary.product_id` references products only; subproduct tracking in conversations is not yet implemented.
